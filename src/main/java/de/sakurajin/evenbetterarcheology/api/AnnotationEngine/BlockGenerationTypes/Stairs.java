...
public class Stairs implements BlockGenerationType {
    ...
    public void generate(String name, DatagenModContainer container, String[] textures) {
        if (textures.length == 1) {
            container.MODEL_GENERATION_HELPER.generateStairs(name, textures[0]);
        }else if (textures.length == 3){
            container.MODEL_GENERATION_HELPER.generateStairs(name, textures[0], textures[1], textures[2]);
        }else{
